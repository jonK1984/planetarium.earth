( function () {
	...
	class TieFighterControls extends THREE.EventDispatcher {
		...
		constructor( object, domElement ) {
			...
			if ( domElement === undefined ) {
				...
				console.warn( 'THREE.TieFighterControls: The second parameter "domElement" is now mandatory.' );
				domElement = document;
			...
			this.keydown = function ( event ) {
				...
				if (document.activeElement === planetSearch) {
					// If typing in the search box, ignore keypresses
					return;
				}
				...
				switch ( event.code ) {
					...
					case 'Equal':
						//Increase Speed +
						this.throttle = Math.min(this.throttle + this.movementSpeedMultiplier, this.maxThrottle);
						this.dispatchEvent(_changeEvent); // Notify throttle change
						break;

					case 'Minus':
						//Decrease Speed -
						this.throttle = Math.max(this.throttle - this.movementSpeedMultiplier, this.maxReverse);
						this.dispatchEvent(_changeEvent); // Notify throttle change
						break;
					
					case 'BracketLeft':
						//Increase Speed by 25% ]
						if( this.throttle > 0 && this.throttle < 25) 
						{
							this.throttle = 0;
							break;
						}
						this.throttle = Math.max(this.throttle - this.maxThrottle * .25, this.maxReverse);
						this.dispatchEvent(_changeEvent); // Notify throttle change
						break;
					case 'BracketRight':
						...
						this.throttle = Math.min(this.throttle + this.maxThrottle * .25, this.maxThrottle);
						this.dispatchEvent(_changeEvent); // Notify throttle change
						break;
			...
			this.mousemove = function ( event ) {
				...
				if ( ! this.dragToLook || this.mouseStatus > 0 ) {
					...
					this.moveState.yawLeft = - this.pitchSpeed * ( event.pageX - container.offset[ 0 ] - halfWidth ) / halfWidth;
					this.moveState.pitchDown = this.pitchSpeed * ( event.pageY - container.offset[ 1 ] - halfHeight ) / halfHeight;
					this.updateRotationVector();
			...
			this.updateMovementVector = function () {
				...
				this.moveVector.z = - this.throttle + this.moveState.back;
	...
	THREE.TieFighterControls = TieFighterControls;
